...
class CIANetWoDDP(nn.Module):
    ...
    def forward(self, x, **kwargs):
        x = net_in = cat([x[:, n:n+1] for n in self.in_group[:-1]], dim=1)
        ...
        saf_in = cat([domain_encoders_out[i][0] for i in range(self.domains)], dim=1)
        baf_out, baf_deep_segs = self.saf_module(saf_in, net_in)
...
if __name__ == '__main__':
    ...
    net = CIANetWoDDP([5, 8], 116).to(device)
