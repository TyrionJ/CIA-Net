...
class DownsampleSegForDSTransform(AbstractTransform):
    def __init__(self, sup_depth,
        ...
        self.order = order
        self.ds_scales = [2 ** -i for i in range(sup_depth)]
    ...
    def __call__(self, **data_dict):
        ...
        for s in self.ds_scales:
            if not isinstance(s, (tuple, list)):
                ...
                assert len(s) == len(axes), f'If ds_scales is a tuple for each resolution (one down-sampling factor ' \
                                            f'for each axis) then the number of entries in that tuple (here ' \
